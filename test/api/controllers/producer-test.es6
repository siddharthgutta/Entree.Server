...
describe('Producer DB API', () => {
  ...
  describe('#updateByObjectId', () => {
    ...
    it('should add hours correctly', async () => {
      const {_id} = await Producer._create(name, 'dominos', password, description, fields.profileImage);
      const day = 'Monday';
      const open = '10:30';
      const close = '22:30';
      await Producer.setHours(day, open, close, _id);
      const producer = await Producer.findOneByObjectId(_id);
    ...
    it('should work tests getHours ', async () => {
      ...
      await Producer.setHours('Wednesday', fields.hour.openTime, fields.hour.closeTime, _id);
      const check = await Producer.getHours(_id);
      assert.equal(check[check.length - 1].day, hours.day);
    });

    it('should work tests deleteHours', async () => {
      const hours = {
        day: 'Wednesday',
        openTime: '07:00',
        newOpen: '8:00',
      ...
      let temp = await Producer.setHours(hours.day, hours.openTime, hours.closeTime, _id);
      const check = temp.hours.length - 1;
      temp = await Producer.deleteHours(_id, hours.day, hours.openTime, hours.closeTime);
      assert.equal(check, temp.hours.length);
    });
    ...
    it('should work tests updateHours', async () => {
      ...
      const {_id} = await Producer._create('Food truck', 'dominos', password, description, 1, 1, {
        merchant: {
          merchantID: '987666'
        }});
      await Producer.setHours(hours.day, hours.openTime, hours.closeTime, _id);
      const temp = await Producer.updateHours(_id, hours.day, hours.openTime,
        hours.closeTime, hours.newOpen, hours.newClose);
      assert.equal(hours.newOpen, temp.hours[temp.hours.length - 1].openTime);
    });
  });
});
